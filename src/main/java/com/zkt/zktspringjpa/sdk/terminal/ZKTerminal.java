package com.zkt.zktspringjpa.sdk.terminal;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.SerializationFeature;
import com.zkt.zktspringjpa.sdk.Enum.*;
import com.zkt.zktspringjpa.sdk.Exception.DeviceNotConnectException;
import com.zkt.zktspringjpa.sdk.command.events.EventCode;
import com.zkt.zktspringjpa.sdk.commands.*;
import com.zkt.zktspringjpa.sdk.utils.HexUtils;
import com.zkt.zktspringjpa.sdk.utils.SecurityUtils;
...
public class ZKTerminal {
    ...
    public List<AttendanceRecord> getAttendanceRecords() throws IOException, ParseException {
        ...
        if (replyCode == CommandReplyCodeEnum.CMD_PREPARE_DATA) {
            System.out.println("CMD_PREPARE_DATA");
            boolean first = true;
        ...
        while (attendance.length() > 0) {
            ...
            // formatting the date in the required format (sql date-time format)

            DateTimeFormatter inpurFormatter = DateTimeFormatter.ofPattern("EEE MMM dd HH:mm:ss zzz yyyy",
                    Locale.ENGLISH);
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

            LocalDateTime dateTime = LocalDateTime.parse(attendanceDate.toString(), inpurFormatter);
            String formattedDate = dateTime.format(outputFormatter);

            // custom date format ends
            ...
            AttendanceRecord attendanceRecord = new AttendanceRecord(seq, userId.trim(), attendanceType, formattedDate,
                    attendanceState);
    ...
    public ZKCommandReply setCommKey(int key) throws IOException {
        ...
        // TODO:
        if (replyCode == CommandReplyCodeEnum.CMD_ACK_OK) {
            System.out.println("COMKey set successfully");
        }
    ...
    // Delete User Finger (Not working )
    public ZKCommandReply delUsertemp(int delUId, int tempId) throws IOException {

        ByteBuffer commandBuffer = ByteBuffer.allocate(3);
        commandBuffer.putShort((short) delUId);
        commandBuffer.put((byte) tempId);

        int[] toSend = ZKCommand.getPacketByte(CommandCodeEnum.CMD_DELETE_USERTEMP, sessionId, replyNo,
                commandBuffer.array());
        byte[] buf = new byte[toSend.length];
        int index = 0;
    ...
    public ZKCommandReply testVoice(int voice) throws IOException {
        ...
        DatagramPacket packet = new DatagramPacket(buf, buf.length, address, port);
        socket.send(packet);
        // System.out.println("Sending CMD_TESTVOICE packet. Payload: " +
        // Arrays.toString(buf));
        replyNo++;
