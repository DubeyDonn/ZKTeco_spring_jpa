package com.zkt.zktspringjpa.sdk.iclockhelper;
...
public class GetRequest extends ZKRequest {
    ...
    // info =
    // String.format("FWVersion=%s\tUserCount=%s\tFPCount=%s\tTransactionCount=%s\tIPAddress=%s\tFPVersion=%s\t",
    // splittedInfo[0], splittedInfo[1], splittedInfo[2], splittedInfo[3],
    // splittedInfo[4], splittedInfo[5]);
    // } else if (splittedInfo.length == 5) {
    // info =
    // String.format("FWVersion=%s\tUserCount=%s\tFPCount=%s\tTransactionCount=%s\tIPAddress=%s\t",
    // splittedInfo[0], splittedInfo[1], splittedInfo[2], splittedInfo[3],
    // splittedInfo[4]);
    // } else if (splittedInfo.length == 4) {
    // info =
    // String.format("FWVersion=%s\tUserCount=%s\tFPCount=%s\tTransactionCount=%s\t",
    // splittedInfo[0], splittedInfo[1], splittedInfo[2], splittedInfo[3]);
    // }
    //
    ...
    private static Info _fillPlainInfo(String info) {
        if (info != null && !info.isEmpty()) {
            ...
            if (splittedInfo.length >= 10) {
                ...
                info = String.format(
                        "FirmwareVersion=%s\tUserCount=%s\tFPCount=%s\tTransactionCount=%s\tIPAddress=%s\tFPAlgorithmVersion=%s\tFaceAlgorithmVersion=%s\tRequiredFaceCount=%s\tEnrolledFaceCount=%s\tFingerprintFunction=%s\tFaceFunction=%s\tUserPhotoFunction=%s",
                        firmwareVersion, userCount, fpCount, transactionCount, ipAddress, fpAlgorithmVersion,
                        faceAlgorithmVersion, requiredFaceCount, enrolledFaceCount, fingerprintFunction, faceFunction,
                        userPhotoFunction);
            }
    ...
    private static Info _fillInfo(String info) {
        ...
        Arrays.stream(Info.class.getDeclaredFields())
                ...
                .forEach(field -> {
                    ...
                    if (Arrays.asList("fpCount", "transactionCount", "userCount", "maxFingerCount", "maxAttLogCount")
                            .contains(normalKey)) {
                        int intValue = 0;
                        try {
